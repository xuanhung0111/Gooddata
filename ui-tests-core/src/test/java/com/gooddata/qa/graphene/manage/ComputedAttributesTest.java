...
public class ComputedAttributesTest extends GoodSalesAbstractTest {
    ...
    public void createComputedAttributeTest() {
        ...
        By computedAttributeItem = By.cssSelector(String.format(".s-title-%s a", 
                CssUtils.simplifyText(COMPUTED_ATTRIBUTE_NAME)));
        ...
        List<String> expectedBucketRanges = Arrays.asList("# of Won Opps. <= 120", "120 < # of Won Opps. <= 200",
    ...
    @Test(dependsOnMethods = { "createProject" }, priority = 0)
    public void checkValidationOfBucketFields() throws InterruptedException {
        initAttributePage();
    ...
    public void checkOtherPagesAfterComputedAttributeCreated() throws InterruptedException, ParseException, 
    ...
    public void applyListFiltersOnReportWithComputedAttribute() throws InterruptedException {
        initReportsPage();
        ...
        List<Float> metricValues = reportPage.getTableReport().getMetricElements();
        assertEquals(attributeHeaders, Arrays.asList(COMPUTED_ATTRIBUTE_NAME), "Attribute name is incorrrect");
        assertEquals(attributeValues, Arrays.asList("Good", "Poor"), 
                "Attribute values are incorrrect " + attributeValues);
    ...
    public void applyVariableFiltersOnReportWithComputedAttribute() throws InterruptedException {
        initVariablePage();
        ...
        createReport(new ReportDefinition().withName(CA_VARIABLE_REPORT_NAME)
                .withWhats("Amount")
    ...
    public void computedAttributeVariableOnDashboard() throws InterruptedException {
        try {
    ...
    @Test(dependsOnMethods = {"createReportWithComputedAttribute"})
    public void computedAttributeReportOnDashboard() throws InterruptedException {
        try {
            ...
            assertEquals(attributeHeaders, Arrays.asList(COMPUTED_ATTRIBUTE_NAME), "Attribute name is incorrrect");
            assertEquals(attributeValues, Arrays.asList("Best", "Great"),
                    "Attribute values are incorrrect " + attributeValues);
    ...
    public void applyMUFFiltersOnReportWithComputedAttribute() throws IOException, JSONException {
        try {
            ...
            initReportsPage();
            reportsPage.getReportsList().openReport(REPORT_NAME);
            Screenshots.takeScreenshot(browser, "editor-user-report-created-with-computed-attribute", this.getClass());
            List<String> attributeHeaders = reportPage.getTableReport().getAttributesHeader();
            List<String> attributeValues = reportPage.getTableReport().getAttributeElements();
            List<Float> metricValues = reportPage.getTableReport().getMetricElements();
            assertEquals(attributeHeaders, Arrays.asList(COMPUTED_ATTRIBUTE_NAME), "Attribute name is incorrrect");
            ...
            String mufURI = RestUtils.createMUFObj(restApiClient, testParams.getProjectId(), "Status User Filters",
                    buildConditions());
    ...
    @Test(dependsOnMethods = { "createReportWithComputedAttribute", "applyListFiltersOnReportWithComputedAttribute",
            "computedAttributeReportOnDashboard", "applyMUFFiltersOnReportWithComputedAttribute" })
    public void deleteAttributeAndMetricUsedInComputedAttribute() throws InterruptedException {
        initAttributePage();
    ...
    public void deleteVariableUsedInComputedAttributeReport() throws InterruptedException {
        initVariablePage();
