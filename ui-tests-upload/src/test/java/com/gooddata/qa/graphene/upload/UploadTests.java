...
public class UploadTests extends AbstractUploadTest {
	...
	public void uploadSpecialUnicodeCharacterColumnName()
		...
		try {
			...
			checkAttributeName("~!@#$%^&*()<>/?;'");
			Screenshots.takeScreenshot(browser,
					"attribute-name-with-special-character", this.getClass());
	...
	public void uploadNegativeNumber() throws InterruptedException {
		try {
			...
			waitForElementVisible(By
					.xpath("//iframe[contains(@src,'Auto-Tab')]"));
			checkUtils.waitForDashboardPageLoaded();
            ui.verifyProjectDashboardsAndTabs(true, expectedDashboardsAndTabs,
					false);
			Screenshots.takeScreenshot(browser,
			...
			List<String> datasets = Arrays.asList("payroll-negative-number",
					"Date (payroll-negative-number_paydate)");
	...
	public void uploadNullNumber() throws InterruptedException {
		try {
			...
			waitForElementVisible(By
					.xpath("//iframe[contains(@src,'Auto-Tab')]"));
			checkUtils.waitForDashboardPageLoaded();
            ui.verifyProjectDashboardsAndTabs(true,
			...
			List<String> how = new ArrayList<String>();
			how.add("Lastname");
			prepareReport("Report with null number", ReportTypes.TABLE, what,
					how);
			...
			List<String> datasets = Arrays.asList("payroll-null-number",
					"Date (payroll-null-number_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	...
	public void uploadChangeColumnType() throws InterruptedException {
		try {
            ...
            ui.uploadCSV(csvFilePath + "change-column-type.csv", columnIndexAndType, "change-column-type");
	...
	public void uploadCheckEmptyColumnName() throws InterruptedException {
		...
		Screenshots.takeScreenshot(browser, "check-empty-column-name-upload",
				this.getClass());
	}
	...
	public void uploadCheckUniqueColumnName() throws InterruptedException {
		...
		Screenshots.takeScreenshot(browser, "check-unique-column-name-upload",
				this.getClass());
	}
	...
	public void uploadIncorrectCSVFile() throws InterruptedException {
		uploadInvalidCSVFile(
				...
				"UNABLE TO IMPORT FILE. NUMBER OF COLUMNS MISMATCH.",
	...
	public void uploadDifferentDate1() throws InterruptedException {
		try {
			...
			List<String> datasets = Arrays.asList("payroll-dd-mm-yy",
					"Date (payroll-dd-mm-yy_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	}
	...
	public void uploadDifferentDate2() throws InterruptedException {
		try {
			...
			List<String> datasets = Arrays.asList("payroll-ddmmyy",
					"Date (payroll-ddmmyy_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	}
	...
	public void uploadDifferentDate3() throws InterruptedException {
		try {
			...
			List<String> datasets = Arrays.asList("payroll-mm-dd-yy",
					"Date (payroll-mm-dd-yy_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	}
	...
	public void uploadDifferentDate4() throws InterruptedException {
		try {
			...
			List<String> datasets = Arrays.asList("payroll-mmddyy",
					"Date (payroll-mmddyy_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	}
	...
	public void uploadDifferentDate5() throws InterruptedException {
		try {
			...
			List<String> datasets = Arrays.asList("payroll-yyyymmdd",
					"Date (payroll-yyyymmdd_paydate)");
			this.cleanDashboardAndDatasets(datasets);
		}
	}
	...
	public void uploadDataWithCrazyFooter() throws InterruptedException {
		try {
			...
			List<String> columnNames = Arrays.asList("Renewal Status",
					...
					"Total first year potential", "Probability (%)",
