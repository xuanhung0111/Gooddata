...
import { newAttribute, newMeasure, newPositiveAttributeFilter, newRelativeDateFilter } from "@gooddata/sdk-model";
...
class App extends Component {
    render() {
        const measure = newMeasure(C.measure(metric),
            f =>f.filters(newRelativeDateFilter(
                C.dateDataSet(dataSet), 'GDC.time.year', parseInt(from), parseInt(to))).alias(metric)
        );

        const viewBy = newAttribute(C.attributeDisplayForm(attribute, displayForm));

        const filters = [newPositiveAttributeFilter(
            C.attributeDisplayForm(attribute, displayForm), {uris: [elementOfAttribute]}
        )];
