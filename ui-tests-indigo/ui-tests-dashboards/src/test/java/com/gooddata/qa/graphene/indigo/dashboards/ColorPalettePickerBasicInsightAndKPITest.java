...
public class ColorPalettePickerBasicInsightAndKPITest extends GoodSalesAbstractTest {
    ...
    public void prepareInsightsApplyColorsPalette() {
        initAnalysePage().addMetric(METRIC_NUMBER_OF_ACTIVITIES)
                ...
                .saveInsight(INSIGHT_HAS_VIEW_BY);

        initAnalysePage().addMetric(METRIC_NUMBER_OF_ACTIVITIES)
                .addStack(ATTR_ACTIVITY_TYPE)
                .saveInsight(INSIGHT_HAS_STACK_BY);
    ...
    protected void testInsightFilterDateBySPPYApplyColorMappingPicker() {
        ...
        analysisPage.openInsight(INSIGHT_FILTER_DATE_BY_SPPY).waitForReportComputing()
                .getFilterBuckets()
                .openDateFilterPickerPanel()
                .applyCompareType(CompareTypeDropdown.CompareType.SAME_PERIOD_PREVIOUS_YEAR);
        analysisPage.waitForReportComputing();
        setColorPickerMapping(analysisPage);
    ...
    protected void testInsightFilterDateBySPPYApplyCustomColorPicker() {
        try {
            ...
            AnalysisPage analysisPage = initAnalysePage();
            analysisPage.openInsight(INSIGHT_FILTER_DATE_BY_SPPY).waitForReportComputing()
                    .getFilterBuckets().openDateFilterPickerPanel()
                    .applyCompareType(CompareTypeDropdown.CompareType.SAME_PERIOD_PREVIOUS_YEAR);
            analysisPage.waitForReportComputing();
            ChartReport chartReport = analysisPage.setCustomColorPicker(ColorPalette.RED.getHexColor())
                    .applyCustomColorPicker().getChartReport();
            assertEquals(chartReport.checkColorColumn(0, 0), ColorPalette.LIGHT_RED.toString());
    ...
    protected void testInsightApplyColorMappingPicker() {
        AnalysisPage analysisPage = initAnalysePage();
        analysisPage.openInsight(INSIGHT_HAS_STACK_BY);
        setColorPickerMapping(analysisPage);
        ChartReport chartReport = analysisPage.getChartReport();
        assertEquals(chartReport.checkColorColumn(0, 0), ColorPalette.BRIGHT_RED.toString());
        assertEquals(chartReport.getLegendColors(), asList(ColorPalette.BRIGHT_RED.toString(), ColorPalette.LIME_GREEN.toString(),
    ...
    protected void testInsightApplyCustomColorPickerWithCancelButton() {
        try {
            setCustomColorPickerFlag(true);
            AnalysisPage analysisPage = initAnalysePage();
            analysisPage.openInsight(INSIGHT_HAS_STACK_BY).waitForReportComputing();
            ChartReport chartReport = analysisPage.setCustomColorPicker(ColorPalette.RED.getHexColor())
                    .clickCancelButtonInColorCustomPicker()
                    .getChartReport();
    ...
    protected void testInsightApplyCustomColorPickerWithOkButton() {
        try {
            ...
            assertEquals(chartReport.getLegendColors(), asList(ColorPalette.RED.toString(), ColorPalette.LIME_GREEN.toString(),
                    ColorPalette.BRIGHT_RED.toString(), ColorPalette.PURE_ORANGE.toString()));
    ...
    protected void resetColorMappingPickerOnAnalyze() {
        AnalysisPage analysisPage = initAnalysePage();
        analysisPage.openInsight(APPLY_COLOR_MAPPING_PICKER_WITH_INSIGHT_HAS_STACK_BY);
    ...
    protected void resetCustomColorPickerOnAnalyze() {
        try {
            setCustomColorPickerFlag(true);
            AnalysisPage analysisPage = initAnalysePage();
            analysisPage.openInsight(APPLY_CUSTOM_COLOR_PICKER_WITH_INSIGHT_HAS_STACK_BY);
            ChartReport chartReport = analysisPage.resetColorPicker().getChartReport();
            assertEquals(chartReport.checkColorColumn(0, 0), ColorPalette.CYAN.toString());
            assertEquals(chartReport.getLegendColors(), asList(ColorPalette.CYAN.toString(), ColorPalette.LIME_GREEN.toString(),
                    ColorPalette.BRIGHT_RED.toString(), ColorPalette.PURE_ORANGE.toString()));
    ...
    protected void resetInsightApplyColorPickerMappingSwitchingBetweenColumnChartAndPieChart() {
        AnalysisPage analysisPage = initAnalysePage();
        analysisPage.openInsight(INSIGHT_HAS_VIEW_BY).waitForReportComputing();
        setColorPickerMapping(analysisPage);
        ...
        ChartReport chartReport = analysisPage.resetColor().getChartReport();
        assertEquals(chartReport.checkColorColumn(0, 0), ColorPalette.CYAN.toString());
        analysisPage.changeReportType(ReportType.PIE_CHART).waitForReportComputing();
        ChartReport pieChartReport = analysisPage.getChartReport();
        assertEquals(pieChartReport.checkColorColumn(0, 0), ColorPalette.BRIGHT_RED.toString());
        assertEquals(chartReport.getLegendColors(), asList(ColorPalette.BRIGHT_RED.toString(), ColorPalette.LIME_GREEN.toString(),
    ...
    protected void resetInsightApplyCustomColorPickerSwitchingBetweenColumnChartAndPieChart() {
        try {
            setCustomColorPickerFlag(true);
            AnalysisPage analysisPage = initAnalysePage();
            analysisPage.openInsight(INSIGHT_HAS_VIEW_BY).setCustomColorPicker(ColorPalette.RED.getHexColor())
                    .applyCustomColorPicker().waitForReportComputing();
            analysisPage.changeReportType(ReportType.PIE_CHART).setCustomColorPicker(ColorPalette.RED.getHexColor())
                    .applyCustomColorPicker().waitForReportComputing();
            analysisPage.changeReportType(ReportType.COLUMN_CHART).waitForReportComputing();
            ChartReport chartReport = analysisPage.resetColor().getChartReport();
            assertEquals(chartReport.checkColorColumn(0, 0), ColorPalette.CYAN.toString());
    ...
    protected void resetColorMappingPickerOnKPI() {
        ...
        indigoDashboardsPage.addDashboard().addInsight(RESET_COLOR_MAPPING_PICKER_WITH_INSIGHT_HAS_STACK_BY).waitForWidgetsLoading()
     ...
     * and then select the palette in position (3)
     *
     * @return AnalysisPage after configured
     */
    private ConfigurationPanelBucket setColorPickerMapping(AnalysisPage analysisPage) {
        return analysisPage.getConfigurationPanelBucket().openColorConfiguration()
                .openColorPicker(0).getItemColorPicker(3);
    }
}
