...
public class AbstractDeployProcesses extends AbstractProjectTest {
    ...
    private static final String DISC_PROJECTS_PAGE_URL = "admin/disc/#/projects";
    ...
    protected void deployInProjectsPage(List<String> projects, String zipFileName,
        ...
        String progressDialogMessage = String.format(DEPLOY_PROGRESS_MESSAGE_IN_PROJECTS_PAGE, zipFileName, processName);
        String failedDeployMessage = String.format(FAILED_DEPLOY_MESSAGE_IN_PROJECTS_PAGE, zipFileName);
        String failedDeployError = String.format(FAILED_DEPLOY_ERROR_IN_PROJECTS_PAGE, zipFileName, processName);
        String successfulDeployMessage = String.format(SUCCESSFUL_DEPLOY_MESSAGE_IN_PROJECTS_PAGE, zipFileName);
        if (isSuccessful) {
            deployProcess(zipFilePath + zipFileName + ".zip", processType, processName, progressDialogMessage,
    ...
    protected void deployInProjectDetailPage(String projectName, String zipFileName,
        ...
        String successfulDeployMessage = String.format(SUCCESSFUL_DEPLOY_MESSAGE_IN_PROJECT_DETAIL_PAGE, processName);
        String failedDeployMessage = String.format(FAILED_DEPLOY_MESSAGE_IN_PROJECT_DETAIL_PAGE, zipFileName,
                processName);
        if (isSuccessful) {
            deployProcess(zipFilePath + zipFileName + ".zip", processType, processName, progressDialogMessage,
                    successfulDeployMessage);
            ...
            deployProcess(zipFilePath + zipFileName + ".zip", processType, processName, progressDialogMessage, null);
            waitForElementVisible(projectDetailPage.getDeployErrorDialog());
            Assert.assertEquals(failedDeployMessage, projectDetailPage.getDeployErrorDialog().getText());
    ...
    protected void redeployProcess(String projectName, String processName, String newZipFileName,
        ...
        String progressDialogMessage = String.format(REDEPLOY_PROGRESS_MESSAGE, processName, newZipFileName,
                newProcessName);
        String successfulDeployMessage = String.format(SUCCESSFUL_REDEPLOY_MESSAGE, processName, newProcessName);
        String failedDeployMessage = String.format(FAILED_REDEPLOY_MESSAGE, newZipFileName, newProcessName);
        if (isSuccessful) {
            deployProcess(zipFilePath + newZipFileName + ".zip", newProcessType, newProcessName, progressDialogMessage,
                    successfulDeployMessage);
            waitForElementVisible(projectDetailPage.getRoot());
